defmodule UnpackTest do
  ...
  describe "unpack/2" do
    test "returns nil if value not found from bad keys", %{player1: player} do
      assert Unpack.unpack(player, [:bogus_key]) == nil
      assert Unpack.unpack(player, [:game, :some_key]) == nil
      assert Unpack.unpack(player, [:game, :some_key, :another_key]) == nil
    end

    test "returns nil if key or value not found in nested attrs", %{player2: player} do
      assert Unpack.unpack(player, [:game, :developer]) == nil
      assert Unpack.unpack(player, [:game, :developer, :some_key]) == nil
      assert Unpack.unpack(player, [:game, :developer, :key, :k2]) == nil
    end

    test "returns value if key found in nested attrs", %{player3: player} do
      assert Unpack.unpack(player, [:game, :developer]) == %{id: "dev-id"}
      assert Unpack.unpack(player, [:game, :developer, :id]) == "dev-id"
    end
  end
end
